...
import 'package:linktobd/view/edit_post.dart';
...
class _FeedCardState extends State<FeedCard> {
  ...
  void initState() {
    ...
    commentCount = int.parse(widget.feedData['comment_count']);
    loopMetaCounts();
  }
                ...
                children: [
                  GestureDetector(
                    onTap: () => Navigator.of(context).push(
                        CustomPageRouteAnimator(child: UserProfile(nickname: '${widget.feedData['nickname']}'), direction: 'fromRight')
                    ),
  ...
  Future<void> loopMetaCounts() async {
    Duration getRandomDuration() {
      final random = Random();
      // Generate a random number between 20,100 and 40,000 milliseconds (20.1 to 40.0 seconds)
    ...
    timer = Timer.periodic(getRandomDuration(), (timer) async {
      ...
      if (response.statusCode == 200) {
        if (respond['success'].toString() == 'yes') {
          ...
          commentCount = int.parse(respond['comment_count'].toString());
          setState(() {});
        }
      }
    });
